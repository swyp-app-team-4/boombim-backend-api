...
@Slf4j
@Transactional
public class SchedulerServiceV1 {
...
//        autoV0();
    ...
    public void sendDailyNotification() {
        ...
        try {
            // 관리자 아이디 나중에 바꾸게씅.!
            alarmService.sendAllAlarm(adminId, SendAlarmRequest.builder().title(messageService.dailyCommunityTitle())
                    .message(messageService.dailyCommunityMessage()).type(AlarmType.COMMUNICATION).build());
    ...
    private void auto() {
        List<Vote> autoVotes = voteRepository.findByVoteStatusAndEndTimeLessThanEqual(VoteStatus.PROGRESS,
                LocalDateTime.now());

        voteRepository.bulkCloseExpired(VoteStatus.PROGRESS, VoteStatus.END, LocalDateTime.now());

        for (Vote autoVote : autoVotes) {
            pushProducer.publishEndVote(EndVoteMessage.builder().voteId(autoVote.getId()).isQuestioner(true).build());
            pushProducer.publishEndVote(EndVoteMessage.builder().voteId(autoVote.getId()).isQuestioner(false).build());
        }
    }
    ...
    private void passivity() {
        ...
        for (Vote passivityVote : passivityVotes) {
            ...
            pushProducer.publishEndVote(
                    EndVoteMessage.builder().voteId(passivityVote.getId()).isQuestioner(true).build());
            pushProducer.publishEndVote(
                    EndVoteMessage.builder().voteId(passivityVote.getId()).isQuestioner(false).build());
        }
    }
    ...
    private List<Member> getBaseMembers(Vote vote) {
        ...
        for (Member m : creators) {
            byId.put(m.getId(), m);
        }
        for (Member m : duplicators) {
            byId.put(m.getId(), m);
        }
    ...
    private List<Member> getAnswerersOnly(Vote vote) {
        ...
        Set<String> baseIds = baseMembers.stream().map(Member::getId).collect(Collectors.toSet());

        // 3) 답변자 불러와서, 1+2에 없는 사람만 필터
        List<Member> answerers = voteAnswerRepository.findMembersByVote(vote);
        return answerers.stream().filter(m -> !baseIds.contains(m.getId())).distinct() // 혹시 쿼리 중복 대비
