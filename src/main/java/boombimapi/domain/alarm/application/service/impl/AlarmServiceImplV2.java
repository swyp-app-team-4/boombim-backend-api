...
@Transactional
@Slf4j
public class AlarmServiceImplV2 implements AlarmService {
    ...
    public List<HistoryResponse> getAlarmHistory(String userId, DeviceType deviceType) {
        Member user = userRepository.findById(userId).orElseThrow(() -> new BoombimException(ErrorCode.USER_NOT_EXIST));
        ...
        List<AlarmRecipient> alarmHistores = alarmRecipientRepository.findAllByMemberAndDeviceTypeOrderByCreatedAtAsc(
                user, DeviceType.valueOf(deviceType.name()));
        ...
        for (AlarmRecipient alarmHistory : alarmHistores) {
            if (!alarmHistory.getDeliveryStatus().equals(DeliveryStatus.FAILED)) {
                result.add(new HistoryResponse(alarmHistory.getId(), alarmHistory.getAlarm().getTitle(),
                        alarmHistory.getAlarm().getType(), alarmHistory.getDeliveryStatus(),
                        alarmHistory.getAlarm().getCreatedAt()));
            }
        }
    ...
    public void updateAlarmRead(String userId, UpdateAlarmStatusReq req) {
        ...
        if (ar == null) {
            throw new BoombimException(ErrorCode.ALARM_NOT_FOUND);
        }
    ...
    public void updateAlarmStatus(String userId) {
        ...
        if (member.isAlarmFlag()) {
            member.updateIsDeactivateAlarmFlag(); // 알림끔
        } else {
            member.updateIsActivateAlarmFlag(); // 알림킴
        }
    ...
    public SendAlarmResponse sendAllAlarmV1(String senderUserId, SendAlarmRequest request) {
        ...
        Alarm alarm = Alarm.builder().title(request.title()).message(request.message()).type(request.type())
                .sender(sender).build();
        alarm.updateStatus(AlarmStatus.QUEUED);
        Alarm saved = alarmRepository.save(alarm);

        // 2) MQ 발행 (즉시 반환)
        NotifyMessage msg = NotifyMessage.builder().alarmId(saved.getId()).title(request.title())
                .body(request.message()).retryCount(0).build();
    ...
    public SendAlarmResponse sendAllAlarm(String senderUserId, SendAlarmRequest request) {
        ...
        try {
            ...
            // 전체 사용자에게 전송
            sendResult = fcmService.sendNotificationToAll(request.title(), request.message(), alarm);
            ...
            log.info("알림 전송 완료: alarmId={}, 성공={}, 실패={}", savedAlarm.getId(), sendResult.successCount(),
                    sendResult.failureCount());

            return SendAlarmResponse.of(savedAlarm, sendResult.successCount(), sendResult.failureCount(),
